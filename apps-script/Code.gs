...
/**
 * App.html은 번들된 JS가 너무 커서(약 800KB) Apps Script 편집기에 한 번에
 * 붙여넣으면 내용이 잘려나가는 사고가 반복되어(줄바꿈을 넣어도 재발함 —
 * 파일 크기 자체가 문제로 보임), 여러 개의 작은 Bundle*.html 조각 파일로
 * 나눠 저장한다. include()는 그 조각을 그대로(가공 없이) 읽어오는
 * 헬퍼이고, App.html은 <?!= include('Bundle1'); ?> 같은 스크립틀릿으로
 * 조각들을 순서대로 이어붙인다 — 이어붙이기는 서버(이 함수)에서 끝나므로,
 * 브라우저는 예전과 동일하게 하나로 합쳐진 스크립트 하나만 받는다.
